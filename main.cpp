...
int main(int argc, char* argv[]) {
    ...
    std::map<std::list<int>, std::list<int>> freqSets = getFrequentItemsets(transactions, params.minSup, false);
    ...
    if (!params.dCharm) {
        Charm charm;
        auto root = new CharmNode(nullptr, nullptr, nullptr);
        for (auto& itemTid : freqSets)
            root->insertChild(new CharmNode(root, new CharmNode::item_set(itemTid.first), new CharmNode::tid_list(itemTid.second)));
        ...
        DCharm dCharm;
        auto root = new DCharmNode(nullptr, nullptr, nullptr, transactions.size());
        for (auto& itemTid : freqSets)
            root->insertChild(new DCharmNode(root, new DCharmNode::item_set(itemTid.first), new DCharmNode::diff_set(itemTid.second), -2));
